...
module receiver (
    ...
    input wire Rx_clk_en,          // Clock enable for controlling reception timing
    ...
    initial begin
        ...
        data = 8'b11111111;           //Anand on Aug 12-2026: lets say init with all 1's
    end
    ...
    always @(posedge clk) begin
        ...
        if (Rx_clk_en && Rx_en) begin  // Only process data if clock is enabled and Rx is not enabled
            case (state)
                RX_STATE_START: begin  // Handle the start bit
                    if (!Rx || sample != 0)  // Check for the start condition, anand: sample!=0 is when still continuing sampling even rx got disturbed i think so
