...
(
    input wire clk,       // Input clock at 50 MHz
    output wire Rx_clk_en,     // Enable signal for the Rx clock, oversampled
    output wire Tx_clk_en      // Enable signal for the Tx clock
);

    // Parameters for calculating the number of clock cycles per baud bit
	 
	 // Anand added on Aug 14-2026
	 
	localparam RX_ACC_MAX = CLK_FREQ / (BAUD_RATE * RX_OVERSAMPLE);
...
//    parameter TX_ACC_MAX = 50000000 / 115200;        // Calculate max count for Tx
    localparam RX_ACC_WIDTH = $clog2(RX_ACC_MAX);     // Determine bit width needed for Rx accumulator
    localparam TX_ACC_WIDTH = $clog2(TX_ACC_MAX);     // Determine bit width needed for Tx accumulator
